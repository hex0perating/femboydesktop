 ...
 *
 * femboywm is designed like any other X client as well. It is
 * driven through handling X events. In contrast to other X clients, a window
 * manager selects for SubstructureRedirectMask on the root window, to receive
 * events about window (dis-)appearance. Only one X connection at a time is
 * allowed to select for this event mask.
 *
 * The event handlers of femboywm are organized in an array which is accessed
...
setup(void)
{
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "femboywm", 3);
...
spawn(const Arg *arg)
{
	...
	if (fork() == 0) {
		...
		fprintf(stderr, "femboywm: execvp %s", ((char **)arg->v)[0]);
		perror(" failed");
		exit(EXIT_SUCCESS);
	}
}
...
updatebars(void)
{
	...
	XClassHint ch = {"femboywm", "femboywm"};
	for (m = mons; m; m = m->next) {
		if (m->barwin)
			continue;
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	|| (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "femboywm: fatal error: request code=%d, error code=%d\n",
...
{
	die("femboywm: another window manager is already running");
	return -1;
}
...
main(int argc, char *argv[])
{
	...
	if (argc == 2 && !strcmp("-v", argv[1]))
		...
		die("usage: femboywm [-v]");
	if (!setlocale(LC_CTYPE, "") || !XSupportsLocale())
		fputs("warning: no locale support\n", stderr);
	if (!(dpy = XOpenDisplay(NULL)))
		die("femboywm: cannot open display");
	checkotherwm();
	setup();
#ifdef __OpenBSD__
